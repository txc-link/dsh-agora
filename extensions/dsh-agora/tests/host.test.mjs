...
test('host registers one slash command, service accessor, API, and IM gateway command', async () => {
  ...
  const ctx = {
    ...
    accessor(name, descriptor) { accessor = { name, descriptor } },
  ...
  assert.equal(accessor.name, 'dshAgora')
  assert.equal(accessor.descriptor.get().snapshot().im.state, 'connected')
...
test('host stays useful when dsh-im has no public command gateway', () => {
  ...
  apply(ctx)
  assert.equal(accessor.descriptor.get().snapshot().im.state, 'unavailable')
})
