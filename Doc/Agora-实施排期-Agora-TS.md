...
| **20. 记忆自动总结 + 角色例行 + Matrix 群协同** | ✅ **code complete, not deployed** | migration 050 新增 `task_memory_summaries` / `routines` / `routine_runs`；`TaskMemorySummaryService` 对终态任务按 SHA-256 指纹幂等写入 GroupMemoryPort，Mem0 配置后由 observation tick 扫描；`RoutineService` + CLI/REST 提供角色 prompt、role_ref、domain、delivery binding、租约和回执；Matrix connector 新增 `/agora task collab|timeline|context`，合并 Core task/timeline/conversation 快照，绑定房间普通消息也回流 Core conversation。Core/provider-neutral，生活/健康/伴侣仍独立根域，provider 源码零改。聚焦 Core 3/3 + DB/Server 路由测试、Agora workspace build、connector 292/292 + build 通过；未部署。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-memory-routine-matrix/`；Architecture: `Doc/03-ARCHITECTURE/2026-09-01-memory-routine-matrix/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-memory-routine-matrix.md`。 |
