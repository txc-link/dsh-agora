...
**Last updated**: 2026-09-01 (Asia/Shanghai, memory/routine/matrix collaboration)
**Owner**: 总工
...
| **15. Governed agent execution foundation** | ✅ **code complete, not deployed** | migration 047 + provider-neutral `TaskSpecRevision → ExecutionBaseline → EvidenceManifest` chain；SHA-256 digests、append-only parent chain、expiry/status checks、idempotency and restart-safe repositories；REST create/list surfaces and CLI revision append/read-only baseline/evidence queries；baseline creation requires a Dashboard human actor when auth is enabled. Focused 9/9 tests, workspace build, core and barrel gates passed. Repository-wide typecheck/lint still has pre-existing test-only issues and Windows SQLite cleanup EPERM. Planning: `Doc/09-PLANNING/TASKS/2026-09-01-governed-agent-execution/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-governed-agent-execution.md`。 |
| **16. Collaboration governance P0** | ✅ **code complete, not deployed** | migration 048 + provider-neutral `CollaborationRequirement → SubTaskSpec + DelegationAuthority → CollaborationPlan` chain；same-task/requirement reference checks、stable SHA-256 digests、idempotent replay、delegation action/depth/expiry constraints；REST resources under `/api/tasks/:taskId/{collaboration-requirements,subtask-specs,delegation-authorities,collaboration-plans}` and CLI `agora collaboration requirement|spec|authority|plan`；CoordinationRun、mutable task subtasks and provider adapters remain execution-layer concerns. Focused 7/7 tests and workspace build passed; Windows SQLite temporary-directory cleanup `EPERM` remains a pre-existing full-suite issue. Planning: `Doc/09-PLANNING/TASKS/2026-09-01-collaboration-governance/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-collaboration-governance.md`。 |
| **17. Action audit and runtime enforcement** | ✅ **code complete, not deployed** | migration 049 + provider-neutral `ActionAttempt → ActionReceipt` append-only audit chain；admission requires same-task plan/baseline references, active unexpired authority and action allow-list；denied admissions persist a `denied` receipt, successful/failed attempts accept one terminal receipt with idempotent replay；REST task-scoped action-attempts/action-receipts routes and CLI `agora audit attempt|receipt` surfaces；Core remains provider-neutral and does not trigger provider side effects. Focused 7/7, workspace build, core/barrel gates and changed-file lint passed；automatic adapter instrumentation/provider readback deferred to the next slice. Planning: `Doc/09-PLANNING/TASKS/2026-09-01-action-audit/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-action-audit.md`。 |
| **18. Runtime dispatch audit instrumentation** | ✅ **code complete, not deployed** | `metadata.action_audit` context 接入 `RuntimeNodeRegistryService`：创建 dispatch 前自动调用 ActionAuditService admission，拒绝不落 runtime dispatch；服务端和 CLI composition 共用 audit service；完成/失败/取消自动写 terminal receipt，证据仅保留 dispatch/evidence refs；未带治理上下文的存量 dispatch 保持兼容。聚焦 runtime/action-audit 10/10、workspace build、core/barrel gates 通过；非 DSH provider 埋点、provider readback/reconciliation 留给下一阶段。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-action-audit-runtime/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-action-audit-runtime.md`。 |
| **19. Core closure 1–3: governed dispatch + task timeline + document chain** | ✅ **code complete, not deployed** | provider-neutral `GovernedDispatchEnvelope` 串起 approved CollaborationPlan、DelegationAuthority、ExecutionBaseline；有 approved/active plan 的 task 启用 strict dispatch admission，CoordinationService 自动转换 envelope。新增 task timeline/stuck view、stale claim auto-expire + CLI/REST takeover；Markdown artifact 增加 parent/version/diff/review metadata、task artifacts/versions/review routes。聚焦 21/21、workspace build、core/barrel gates、改动文件 lint 通过；provider 源码未改。Windows server app 全量测试仍有既有 SQLite `EPERM` 清理干扰。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-core-closure-1-3/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-core-closure-1-3.md`。 |
| **20. 记忆自动总结 + 角色例行 + Matrix 群协同** | ✅ **code complete, not deployed** | migration 050 基础表 + migration 051 为 `routine_runs` 增加 runtime dispatch、结果、artifact 和投递状态；`TaskMemorySummaryService` 对终态任务按 SHA-256 指纹幂等写入结构化事实/决策/经验/未决摘要，带脱敏元数据；`RoutineRunner` 通过 provider-neutral runtime/artifact/IM ports 完成 claim → dispatch → reconcile → artifact → delivery，失败投递可重试；Matrix connector 新增显式 @角色唤醒、有限轮次、cooldown、event 去重和 bot 循环抑制，同时保留绑定房间 conversation ingest。Core/provider-neutral，生活/健康/伴侣仍独立根域，provider 源码零改。聚焦 Core/DB/Server/CLI + connector tests、Agora workspace build、connector build/test 通过；未部署。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-routine-runner-collab-memory/`；Architecture: `Doc/03-ARCHITECTURE/2026-09-01-memory-routine-matrix/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-routine-runner-collab-memory.md`。 |
...
- 2026-09-01: **Governed agent execution foundation** — 在独立 worktree `feat/governed-agent-execution`（基于 `d17db47`）完成 migration 047、`TaskSpecRevision`/`ExecutionBaseline`/`EvidenceManifest` contracts + SQLite repositories + Core service；校验不可变修订父链、精确 SHA-256 引用、过期/状态边界及幂等重放。`apps/server` 新增六个 REST route；`apps/cli` 新增 `execution revision append|list|show` 与 baseline/evidence list/show，避免 CLI 伪造 Dashboard 审批。聚焦测试 9/9、workspace build、core architecture/barrel gates 通过；未部署、未推送。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-governed-agent-execution/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-governed-agent-execution.md`。
- 2026-09-01: **Collaboration governance P0** — 在独立 worktree `feat/collaboration-governance`（基于 `77dea6d`）完成 migration 048、`CollaborationRequirement`/`SubTaskSpec`/`DelegationAuthority`/`CollaborationPlan` contracts + SQLite repositories + Core service；约束同 task/requirement 组合、引用唯一性、SHA-256 内容摘要、幂等重放、delegation action allow-list/depth/expiry。`apps/server` 暴露四组 REST create/list 资源；`apps/cli` 暴露 `collaboration requirement|spec|authority|plan` append/list/show/propose；不改变 CoordinationRun、mutable subtasks 或 provider adapter 的执行职责，也不绕过 Dashboard 人工审批。聚焦 7/7、workspace build、改动文件 lint 通过；全量 DB 测试仍受 Windows SQLite 清理 `EPERM` 影响；未部署。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-collaboration-governance/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-collaboration-governance.md`。
- 2026-09-01: **Action audit and runtime enforcement** — 在独立 worktree `feat/action-audit`（基于 `086b034`）完成 migration 049、`ActionAttempt`/`ActionReceipt` contracts + SQLite repositories + Core `ActionAuditService`；把协同计划、执行基线、委派授权约束接到一次 provider-neutral admission，拒绝也生成 denied receipt，成功/失败只允许一个终态回执并支持幂等重放。`apps/server` 新增 task-scoped attempt/receipt REST，`apps/cli` 新增 `agora audit attempt|receipt`；不把 Matrix/OpenClaw/Hermes/provider 语义放入 Core，也不自动触发外部副作用。聚焦 7/7、workspace build、core/barrel gates、改动文件 lint 通过；未部署，既有 adapter 自动埋点和 provider readback 留给下一阶段。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-action-audit/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-action-audit.md`。
- 2026-09-01: **Runtime dispatch audit instrumentation** — 在独立 worktree `feat/action-audit-runtime`（基于 `2a9466e`）将 `metadata.action_audit` 接入 `RuntimeNodeRegistryService`：dispatch 创建前自动 admission，成功后持久化 `attempt_id`，拒绝不创建 runtime dispatch；完成/失败/取消按 dispatch/evidence refs 自动生成单一 terminal receipt，重复回调幂等。server/CLI composition 共用 `ActionAuditService`，存量无治理 metadata 的 dispatch 保持兼容；非 DSH provider 埋点和 provider readback 留给下一阶段。聚焦 10/10、workspace build、core/barrel gates 通过；未部署。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-action-audit-runtime/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-action-audit-runtime.md`。
- 2026-09-01: **Core closure 1–3** — 在独立 worktree `feat/core-closure-1-3` 完成 governed dispatch envelope + strict mode、task timeline/stuck view、stale claim takeover、Markdown artifact version/diff/review chain；REST/CLI 入口和 coordination 自动接线，provider 源码零改动。聚焦 21/21、workspace build、core/barrel gates、改动文件 lint 通过；未部署。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-core-closure-1-3/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-core-closure-1-3.md`。
- 2026-09-01: **记忆自动总结 + 角色例行 + Matrix 群协同** — migration 050 为终态任务记忆总结与长期 Routine 增加持久化；`TaskMemorySummaryService` 按 SHA-256 fingerprint 幂等写入 GroupMemoryPort，配置 Mem0 后由 observation tick 自动扫描；`RoutineService`/CLI/REST 支持 role_ref、人格 prompt、domain、Matrix delivery binding、租约和回执；connector `/agora task collab|timeline|context` 合并 Core task/timeline/conversation，让人类与多个 agent 在同一房间回看共享上下文。未改变 provider 源码，生活/健康/伴侣继续独立根域。Core/DB/Server focused tests、workspace build、connector 291/291 通过；未部署。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-memory-routine-matrix/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-memory-routine-matrix.md`。
- 2026-09-01: **RoutineRunner + Matrix 回合控制 + 结构化记忆** — migration 051 将 routine run 关联 runtime dispatch、结果、artifact、delivery status；Core `RoutineRunner` 复用 runtime/artifact/IM 端口完成可重启执行闭环，`POST /api/routines/run` 与 CLI `routine run` 可触发；终态记忆摘要新增四段结构化字段、质量置信度和敏感信息脱敏；connector 对显式 @角色唤醒实施回合上限、cooldown、去重和 agent 普通消息抑制，避免 bot 无限互答。聚焦 Core 7/7、DB routine 2/2、Server 路由 1/1、workspace build、connector 295/295 + build 通过；Windows DB 全套测试受临时目录 EPERM 环境限制，未部署。Planning: `Doc/09-PLANNING/TASKS/2026-09-01-routine-runner-collab-memory/`；Walkthrough: `Doc/10-WALKTHROUGH/2026-09-01-routine-runner-collab-memory.md`。
