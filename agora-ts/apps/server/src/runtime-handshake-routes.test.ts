...
describe('runtime handshake route', () => {
  it('negotiates plugin compatibility', async () => {
    ...
    const response = await app.inject({ method: 'POST', url: '/api/runtime-nodes/node-mac/handshake', payload: { protocol: 'dsh-agora.node/v1', plugin_version: '0.7.0', instance_id: 'i', capabilities: ['heartbeat', 'dispatch', 'delivery'] } });
